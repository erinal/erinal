...
class Admin extends ActiveRecord {
    ...
    public function rules() {
        return [
            ...
            ['adminemail','validateEmail','on' => 'seekpass'] ,//验证这个电子邮箱到底是不是这个用户的
    ...
    public function validatepass() {
        if(!$this->hasErrors()) {
            $data = self::find()->where('adminuser = :user and adminpass = :pass',[":user" => $this->adminuser,":pass" => md5($this->adminpass.Yii::$app->params['salt'])])->one();
            if(is_null($data)) {
    ...
    public function seekPass($data) {
        ...
        if($this->load($data) && $this->validate()) {
            ...
            $mailer->setSubject("这是使用Yii2找回密码的一封测试信");
            if ($mailer->send()) {
                return true;
            }
        }
    ...
    public function reg($data) {
        ...
        if($this->load($data) && $this->validate()) {
            $this->adminpass = md5($this->adminpass.Yii::$app->params['salt']);
            $this->repass = md5($this->adminpass.Yii::$app->params['salt']);
            $this->createtime = time();
            //如果save第一个参数是false那么就不会自动进行验证
            if($this->save(false)) {
                return true;
            }
            return false;
